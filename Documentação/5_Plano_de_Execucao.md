...
| | Definição da estratégia de branches (GitFlow). | Git/GitHub | Arquiteto/Dev | Branches `main` e `develop` criadas e documentadas. |
...
| | Criação de um Vault para segredos (ex: GitHub Secrets). | GitHub | Arquiteto/Dev | Local seguro para armazenar tokens de API e credenciais. |
...
| | Configuração do pipeline de Integração Contínua (CI). | GitHub Actions | DevOps/Dev | Workflow de CI que roda testes a cada PR. |
...
| **7. Publicação** | Configuração do pipeline de Entrega Contínua (CD). | GitHub Actions | DevOps/Dev | Workflow de CD para deploy em ambiente de homologação/produção. |
